...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ result, language }) => {
    ...
    return (
        <div className="space-y-8">
            <Card title={patientSummary.title} icon={<User className="h-6 w-6 text-primary-600 dark:text-primary-300" />}>
                <p>{patientSummary.summary}</p>
            </Card>

            <Card title={physicianReport.title} icon={<Stethoscope className="h-6 w-6 text-primary-600 dark:text-primary-300" />}>
                <p>{physicianReport.introduction}</p>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {physicianReport.resultsTable.map((row, index) => (
                                <tr key={index}>
                                    <td className="px-4 py-3 font-medium whitespace-nowrap">{row.test}</td>
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap font-semibold">
                                        <span className={`${row.interpretation.toLowerCase() === 'high' || row.interpretation.toLowerCase() === 'مرتفع' ? 'text-red-500' : row.interpretation.toLowerCase() === 'low' || row.interpretation.toLowerCase() === 'منخفض' ? 'text-blue-500' : ''}`}>
                                            {row.interpretation}
                                        </span>
                                    </td>
                                </tr>
                ...
                <p>{physicianReport.advancedAnalysis}</p>
            </Card>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                     <RecommendationCard title={recommendations.general.title} icon={<HeartPulse className="w-5 h-5 text-green-600"/>} points={recommendations.general.points} />
                     <RecommendationCard title={recommendations.nutritional.title} icon={<Utensils className="w-5 h-5 text-orange-600"/>} points={recommendations.nutritional.points} />
                     <RecommendationCard title={recommendations.physicalTherapy.title} icon={<Dumbbell className="w-5 h-5 text-blue-600"/>} points={recommendations.physicalTherapy.points} />
                </div>
            </div>
        </div>
    );
...
const RecommendationCard: React.FC<RecommendationCardProps> = ({title, icon, points}) => (
